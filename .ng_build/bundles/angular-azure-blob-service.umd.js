...
var BlobService = (function () {
    ...
    BlobService.prototype.commitBlockList = function (state) {
        ...
        var /** @type {?} */ headers = new http.Headers({ 'x-ms-blob-content-type': state.file.type });
    ...
    BlobService.prototype.upload = function (config) {
        ...
        reader.onloadend = function (evt) {
            if (evt.target.readyState === 2 && !state.cancelled) {
                ...
                var /** @type {?} */ headers = new http.Headers({ 'x-ms-blob-type': 'BlockBlob', 'Content-Type': 'application/octet-stream' });
                var /** @type {?} */ options = new http.RequestOptions({ headers: headers });
                // console.log(uri);
